...
<header class="bg-gradient-to-r from-white to-white p-4 flex justify-between items-center shadow-md">
    <div>
        <a href="{{ route('food.index') }}" class="text-xl">
...
<main class="p-4 space-y-4">
    ...
    <section class="px-6 py-6 pb-10 rounded-lg bg-white border border-slate-200 shadow-lg">
        <form action="{{ route('food.update', encrypt($row->id)) }}" class="" method="POST">
            @csrf
            @method('PUT')
            ...
            <input type="number" name="stock" value="{{ old('stock', $row->stock) }}" placeholder="Stock" class="w-full border border-slate-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-200 focus:border-blue-400 mb-6"/>

            <small class="text-rose-500">@error('price'){{ $message }}@enderror</small>
            <input type="number" name="price" value="{{ old('price', $row->price) }}" placeholder="Serial Number" class="w-full border border-slate-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-200 focus:border-blue-400 mb-6"/>
...
<div id="addModal" class="fixed inset-x-0 bottom-0 top-16 z-50 bg-white rounded-t-2xl p-4 shadow-xl transform translate-y-full transition-transform duration-300 ease-out">
    ...
    <form action="{{ route('food.destroy', encrypt($row->id)) }}" class="space-y-3" method="POST">
        @csrf
        @method('DELETE')
